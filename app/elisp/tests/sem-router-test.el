...
(ert-deftest sem-router-test-task-security-expansion-rejected-before-restore ()
  "Test expansion detection runs before restore and rejects output."
  (require 'sem-llm)
  (let ((verify-called nil)
        (restore-called nil)
        (write-called nil)
        (llm-called nil)
        (context-security-blocks nil)
        (stale-barrier-called nil)
        (mark-processed-calls 0)
        (log-error-call nil)
        (callback-success :unset)
        (sem-core--batch-id 8)
    ...
    (cl-letf (((symbol-function 'org-id-new)
              ...
              ((symbol-function 'require)
               (lambda (feature &optional _filename _noerror)
                 (if (memq feature '(sem-llm sem-security))
                     feature
                   (if (featurep feature) feature t))))
              ((symbol-function 'sem-llm-request)
               (lambda (_prompt _system callback context &optional _tier)
                 (let ((context-with-blocks
                        (plist-put (copy-sequence context) :security-blocks security-blocks)))
                   (setq llm-called t)
                   (setq context-security-blocks (plist-get context-with-blocks :security-blocks))
                   (funcall callback
                            "LLM leaked #+begin_sensitive\nsecret\n#+end_sensitive"
                            nil
                            context-with-blocks))
                 nil))
              ((symbol-function 'sem-router--build-task-llm-prompts)
               (lambda (_title _tags _body _id)
                 (list :user-prompt "u" :system-prompt "s")))
              ((symbol-function 'sem-security-verify-tokens-present)
               (lambda (_output _blocks)
                 (setq verify-called t)
                 (list (cons 'missing nil) (cons 'expanded '("<<SENSITIVE_1>>")))))
              ((symbol-function 'sem-security-restore-from-llm)
               (lambda (output _blocks)
      ...
      (sem-router--route-to-task-llm
       ...
       8)
      (should llm-called)
      (should context-security-blocks)
      (should-not stale-barrier-called)
      (should verify-called)
