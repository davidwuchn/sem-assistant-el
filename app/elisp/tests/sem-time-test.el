...
(ert-deftest sem-time-test-format-string-uses-client-timezone ()
  "Test format helper applies CLIENT_TIMEZONE via time zone rule."
  (let ((set-time-zone-args '())
        (captured-zone nil))
    (cl-letf (((symbol-function 'sem-time-client-timezone)
               (lambda () "America/New_York"))
              ((symbol-function 'getenv)
               (lambda (_name) "Etc/UTC"))
              ((symbol-function 'set-time-zone-rule)
               (lambda (value)
                 (push value set-time-zone-args)))
              ((symbol-function 'format-time-string)
               (lambda (_format _time &optional zone)
                 (setq captured-zone zone)
                 "ok")))
      (should (string= (sem-time-format-string "%Y-%m-%d") "ok"))
      (should (equal (nreverse set-time-zone-args)
