...
This capability defines the Elfeed update mechanism that refreshes the feed database before RSS digest generation.

## Requirements

### Requirement: Elfeed update runs four times before digest generation
The system SHALL refresh the Elfeed database four times before digest generation to ensure fresh feed content.
The cron schedule SHALL be: `0 5 * * *`, `0 6 * * *`, `0 7 * * *`, `0 8 * * *` — one
`emacsclient -e "(sem-rss-elfeed-update-cron)"` call per hour from 5AM to 8AM.

#### Scenario: Update at 5AM
- **WHEN** the cron triggers at 5:00 AM
- **THEN** `emacsclient -e "(sem-rss-elfeed-update-cron)"` is executed
...
### Requirement: elfeed update wrapper called via emacsclient only
The system SHALL call `sem-rss-elfeed-update-cron` exclusively via `emacsclient` from cron.
Emacs internal timers SHALL NOT be used for this purpose.

#### Scenario: Cron invokes update wrapper
- **WHEN** the scheduled time arrived
- **THEN** cron runs `emacsclient -e "(sem-rss-elfeed-update-cron)"`

#### Scenario: No internal timer triggers update
- **WHEN** the daemon is running
- **THEN** no `run-at-time` or `idle-timer` triggers `sem-rss-elfeed-update-cron`
...
### Requirement: feeds.org read via internal feeds parser
The system SHALL read feed subscription list from `/data/feeds.org` via an internal feeds parser.
This file configures which RSS/Atom feeds Elfeed subscribes to.

#### Scenario: Feeds loaded from feeds.org
- **WHEN** `sem-rss-refresh-feeds` runs
- **THEN** it parses subscriptions from `/data/feeds.org`
