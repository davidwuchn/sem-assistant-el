...
This capability defines the body content handling for `:task:` headlines that appends body content to LLM prompts and manages security sanitization.
...
- **WHEN** processing a `:task:` headline with `:body` containing `"Task description details"`
...
- **WHEN** processing a `:task:` headline with `:body` equal to `nil`
- **THEN** the LLM prompt contains no `BODY:` section

#### Scenario: Body sanitized before LLM send
- **WHEN** processing a `:task:` headline with body containing `#+begin_sensitive\nsecret\n#+end_sensitive`
...
- **WHEN** the LLM returns a response for a `:task:` headline that had body content
...
- **WHEN** the LLM returns a response for a `:task:` headline that had no body content
...
- **WHEN** `sem-llm-request` callback is invoked for a `:task:` headline with body
