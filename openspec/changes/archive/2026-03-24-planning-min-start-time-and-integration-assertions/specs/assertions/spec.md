...
- **THEN** all 5 assertions MUST run even if one fails
...
#### Scenario: Scheduled time lower-bound assertion
- **WHEN** running the scheduled-time lower-bound assertion
- **THEN** each generated scheduled task except `Process quarterly financial reports` MUST be strictly greater than `runtime_now + 1 hour`
- **AND** `Process quarterly financial reports` MUST exactly match the scheduled timestamp defined in `dev/integration/testing-resources/inbox-tasks.org`
- **AND** if a violation occurs, the failure message MUST identify the task and the compared timestamps
