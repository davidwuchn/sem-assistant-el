...
### Requirement: Production credential guidance never recommends non-compliant passwords
Production-focused bootstrap and deployment guidance MUST NOT include default or example WebDAV passwords that fail production policy requirements.

#### Scenario: Production examples satisfy password policy
- **WHEN** documentation presents production WebDAV credential examples
- **THEN** each example satisfies the production password policy
- **AND** no production path suggests a credential that would fail startup validation
