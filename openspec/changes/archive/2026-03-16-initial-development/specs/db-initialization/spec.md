...
The system SHALL always delete `/data/org-roam/org-roam.db` (and `*.db-shm`, `*.db-wal`) if they exist, then call `(org-roam-db-sync)` to rebuild from all `.org` files in `/data/org-roam/`.
...
- **WHEN** `/data/org-roam/` contains `.org` files placed before first startup
- **THEN** those files are indexed by `org-roam-db-sync`

#### Scenario: Empty org-roam produces empty DB
- **WHEN** `/data/org-roam/` is empty or does not exist
