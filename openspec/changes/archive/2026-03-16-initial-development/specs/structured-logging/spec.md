...
The `sem-core-log` function SHALL never raise an error itself. All file I/O SHALL be wrapped in `condition-case` and fall back to `message` if the log file is unwritable.
